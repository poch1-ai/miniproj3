...
You can download this .qmd file from [here](https://github.com/joeroith/264_spring_2026/blob/main/11_web_scraping.qmd).  Just hit the Download Raw File button.
...
omdb_api_key <- readLines("omdb_api_key.txt")
```
...
A **second approach** is to use *environment variables* (I have admittedly had mixed results with this approach in the past):
...
```{r}
#omdb_api_key <- readLines("/Users/sokmeanpoch/Data Science 2/data science 2-2/Class activities /omdb_api_key.txt")
#print(omdb_api_key)
 #omdb_api_key <- readLines("~/R/SDS264/Inclass_264/OMDB_KEY.txt")
...
for(i in 1:5) {
  ...
  omdb[i,5] <- as.numeric(details$Matascore)  # metascore (be sure it is numeric)
  omdb[i,6] <- as.numeric(details$imdbRating)  # imdb rating (be sure it is numeric)
  omdb[i,7] <- as.numeric(details$BoxOffice)  # box office (be sure it is numeric)
}
...
# another test with different county/year/variables
rice_test <- MN_tract_data(
  year = 2020,
  county = "Rice",
  variables = c("B25077_001", "B02001_002")
)

rice_test

# build Rice County data for 2019-2021
years <- 2019:2021
...
rice_data |>
  as_tibble() |>
  ggplot(aes(x = factor(year), y = B19013_001E)) +
  geom_boxplot() +
  labs(
    title = "Rice County Median Household Income by Tract, 2019-2021",
    x = "Year",
...
rice_data |>
  as_tibble() |>
  ggplot(aes(x = factor(year), y = B01003_001E)) +
  geom_boxplot() +
  labs(
    title = "Rice County Population by Tract, 2019-2021",
    x = "Year",
    y = "Population"
  )
```
