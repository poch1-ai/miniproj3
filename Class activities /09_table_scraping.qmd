...
You can download this .qmd file from [here](https://github.com/joeroith/264_spring_2026/blob/main/09_table_scraping.qmd).  Just hit the Download Raw File button.
...
library(tidyverse) 
library(rvest) # helps R read HTML 
library(polite) # 
```
...
result <- scrape(session, query = list(t = "semi-soft", per_page = 100)) |> 
  html_node("#main-body") |>  # find the certain section of the page, identify the main body and the 3rd header 
  html_nodes("h3") |> 
  html_text(). # html for plain text 
head(result)
...
result <- scrape(session) |> #scrap from that url
...
*Sicks the two scraped tables side by side because the clamate information was split across 2 tables on the webside*
```{r}
bind_cols(mpls_data1, mpls_data2) |> # combind 2 data frame into 1 wide data frame 
  as_tibble() |>  # convert result into tibble
  select(-`...8`) |> # remove column named ...8
  mutate(`...1` = str_extract(`...1`, "[^ ]+ [^ ]+ [^ ]+")) |> # extract the1st 3 words 
  pivot_longer(cols = c(`JanJa`:`DecDe`), # change the data frame from wide to long
               names_to = "month", values_to = "weather") |> # variable name is the month and the value in the weather 
...
```
combine two scraped climate tables
turn into tibble
remove junk column
clean the row-label column
convert month columns into rows
convert weather types into columns
clean month names
rename variables
...
1. In 12_maps.qmd we will see how to create an interactive U.S. map showing population densities by state.  Right now, let's see if we can use our new web scraping skills to scrape the correct population density data and create a tidy data frame!
...
# Step 0: politely open session
session <- bow("https://en.wikipedia.org/wiki/List_of_states_and_territories_of_the_United_States_by_population_density",
               force = TRUE)

# Step 1-3: scrape all tables from the page
dens_tables <- scrape(session) |>
  html_nodes("table") |>
  html_table(header = TRUE, fill = TRUE)

# Look through the tables first
dens_tables

density_raw <- dens_tables[[1]]
density_raw

density_tidy <- density_raw |>
  as_tibble() |>
  select(State = 1, Density = 3) |>
  mutate(
...
pop3 <- as_tibble(pop2[,c(1:6,8)]) |>
  ...
  mutate(Estimate2024 = parse_number(Estimate2024),
         ...
         Change =  str_extract(Change, (".\\d..[^//%]"),  # get rid of % but preserve +/-,
...
outliers <- pop3 |> 
  filter(Estimate2024 > 300000 | Area > 500)
...
```{r}
"https://www.hockey-reference.com/teams/" + team + "/" + year + ".html"
```
...
hockey_stats <- function(team, year) {
  ...
  url <- str_c("https://www.hockey-reference.com/teams/",
               team, "/", year, ".html")
  
  session <- bow(url, force = TRUE)
  
  tables <- scrape(session) |>
    html_nodes("table") |>
    html_table(header = TRUE, fill = TRUE)
  
  # inspect tables first if needed
  stats_raw <- tables[[1]]
...
hockey_stats <- function(team, year) {
  ...
  # Select correct table (adjust if needed)
  stats_raw <- tables[[1]]
...
```{r}
years <- 2001:2004
teams <- rep("MIN", length(years))

wild_2001_2004 <- map2(teams, years, hockey_stats) |>
  list_rbind()
