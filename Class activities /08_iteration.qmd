...
You can download this .qmd file from [here](https://github.com/joeroith/264_spring_2026/blob/main/08_iteration.qmd).  Just hit the Download Raw File button.
...
```{r}
#| error: TRUE
# Generate random 10 numbers, create tibble a, b, c, d 
df <- tibble(
  a = rnorm(10),
  b = rnorm(10),
  c = rnorm(10),
...
# want median of each column (w/o cutting and pasting), you cun't take the median of a list 
#   Be careful using square brackets vs double square brackets when
#   selecting elements
median(df[[1]]) 
median(df[1]) # you cant take the median from a list 

df[1]    # tibble with 1 column 
df[[1]]  # numeric vector 
...
output <- vector("double", ncol(df))  # 1. output, creates numeric vector of length equal to number of columns, ncols(df)how many spaces you need to fill, number of column
for (i in 1:4) {                      # 2. sequence, loop over column indices 1,2,3,4. For i in 1 to 4
  output[[i]] <- median(df[[i]])      # 3. body, for each column i, compute its median, store output as vector, 
}
output

# ?seq_along - a safer option if had zero length vectors, otherwise identical 
...
means <- c(0, 1, 2)  # means is a vector of 3 number so you loop 3 times 
output <- double()   # output starts empty, you define output as double 
for (i in seq_along(means)) { # sequence is i along means, how long is the mean, runs 3 time. Generate 1 to 3
  n <- sample(100, 1) # pick random number between 1, 100. 1 sample is run 1 to 100. Random sample size 
  output <- c(output, rnorm(n, means[[i]])) # rnorm create random number for a normal distribution, tells us how many random normal we want.
}
# Run 3 times 
...
for (i in seq_along(means)) {
  n <- sample(100, 1)
  out[[i]] <- rnorm(n, means[[i]]) # generates n random numbers wth mean = means1}
}
...
**[Pause to Ponder:]** What does the following code do?
 You flip a coin repeatedly until you get 3 heads in a row
```{r}
flip <- function() sample(c("T", "H"), 1) # returns "H" or "T" randomely, either head or tail
flips <- 0 # count total flip, starts with 0 flip
nheads <- 0 # count consecutive heads, starts wit 0 head 
while (nheads < 3) {
  if (flip() == "H") {
    nheads <- nheads + 1 # if you flip head, head inlease by 1
  } else {
    nheads <- 0 # if you get tails, head will be reset to 0
  }
  flips <- flips + 1 # always inclease the number of flip by 1
}
# flip the coin. If it's head, you increase nheads. If it's tails, you reset heads to 0, Always increase flip 
flips # how many flips it took to get 2 heads in a row
```
while loop can keep runing and runing
...
```{r}
simulated_diffs <- numeric(1000)
for (i in 1:1000) {
  shuffle_data <- dolphin_data |>
    mutate(treatment = sample(treatment))
  
  # recompute proportions
  sim_summary <- shuffled_data |>
    group_by(treatment) |>
    summarize(prop_yes = mean(improve == "Yes"))
  
  # store difference
  simulated_diffs[i] <- sim_summary[[2]][2] - sim_summary[[2]][1]
}
  # Shuffle lables, pretent treatment doesn't matter 
  # reclaulate the differnt
 # store result, after 1000 loops, you have 1000 fake differecesfrom the null world
...
for (j in 1:1000) {
  ...
  group_by(treatment) |>
  summarize(prop_yes = mean(improve == "Yes"))
  simulated_diffs[[j]] <-dolphin_summary[[2]][2] - dolphin_summary[[2]][1]
}
...
for (i in 1:1000) {
  ...
  # shuffle treatment labels (this creates the "null world")
  shuffled_data <- dolphin_data |>
    mutate(treatment = sample(treatment))
...
# define a fuction that applies a summery fuction to each column
col_summary <- function(df, fun) { 
  out <- vector("double", length(df))  # make an empty numeric vector with 1 slot per column, howver long the data set is  
  for (i in seq_along(df)) { # loop through the column position 1,2,3..., perform that fuction in that i column
    out[i] <- fun(df[[i]])
  }
  out
}
col_summary(df, median) # apply median to each column 
col_summary(df, mean) # apply mean to each column 
col_summary(df, IQR) # apply IQR to each column 
```
Put for loop into a fuction and let it functionize it
*df is the data, fun is a fuction like mean, media, IQR*
...
map_dbl(df, mean) # apply mean to each collumn and return a double/ numeric vector
map_dbl(df, median)
map_dbl(df, sd)
...
df |> summarize(
  n = n(),
  across(.cols = a:d, .fns = median, .names = "median_{.col}") # apply median to columns a through d 
)
...
df_miss <- df
df_miss[2, 1] <- NA # put A in rows 2 of column 1 ( column a)
df_miss[4:5, 2] <- NA # put NA in rows 4 and 5 of column 2 ( column b)
df_miss
df_miss |> 
  summarize(
    across(
      a:d,
      list(
        median = \(x) median(x, na.rm = TRUE), # / x use this one inside a looop or anothe function but never gonna use again, temporary fuction, never be used outside of it
...
models <- split(mtcars, mtcars$cyl) |>
  map(function(df) lm(mpg ~ wt, data = df)) # map this fuction to each of thoes empty car 
models
models[[1]]
...
# read_excel("https://joeroith.github.io/264_spring_2026/Data/gapminder/1952.xlsx")
...
```{r}
purrr::map_int(iris, ~length(unique(.x)))

iris |> 
  summarize(across(everthing(), ~length(unique(.x))))
```
...
show_missing <- function(df, group_vars, summary_vars = everything()) {
  df |> 
    group_by(pick({{ group_vars }})) |>  #Dynamically groups the data by the variables the user supplies (e.g., year, month, day).
    summarize(
      across({{ summary_vars }}, \(x) sum(is.na(x))), # For each group, counts how many missing values appear in each selected column.
      .groups = "drop" # ungroup the result 
    ) |>
    select(where(\(x) any(x > 0))) # Keeps only columns where at least one group has missing values.(Columns with zero missing values everywhere are removed.)
}
...
summary_stats <- function(df, vars, stat_fcts) {
  df |>
    summarize(
      across(
        {{ vars }},
...
a. generate simulated data where there is a true difference or effect
...
# store results (TRUE/FALSE if CI contains 25)
contains_mean <- rep(FALSE, num_samples)
...
for (i in 1:num_samples) {
  ...
  contains_mean[i] <- (lower <= true_mean) & (true_mean <= upper)
}

# proportion of intervals that contain 25
mean(contains_mean)
```
