...
You can download this .qmd file from [here](https://github.com/joeroith/264_spring_2026/blob/main/10_web_scraping.qmd).  Just hit the Download Raw File button.
...
Please see `09_table_scraping.qmd` for a preview of web scraping techniques when no API exists, along with ethical considerations when scraping data.  In this file, we will turn to scenarios when the webpage contains data of interest, but it is not already in table form.  
...
Scroll through the page to verify that only the information you intend (the description paragraph) is selected. The selector panel shows the CSS selector (`.thumbnail-teaser__heading`) and the number of matches for that CSS selector (10). (You may have to be careful with your clicking--there are two overlapping boxes, and clicking on the link of the title can lead to the CSS selector of "a".)
...
- The images for each of the news releases


### Retrieving Data Using `rvest` and CSS Selectors

Now that we have identified CSS selectors for the information we need, let's fetch the data using the `rvest` package similarly to our approach in `10_table_scraping.qmd`.
...
Finding the exact node (e.g. ".thumbnail-teaser__heading") is the tricky part.  Among all the html code used to produce a webpage, where do you go to grab the content of interest?  This is where SelectorGadget comes to the rescue!

```{r}
# Step 2: Extract specific nodes with html_nodes()
title_temp <- html_nodes(nih, ".thumbnail-teaser__heading")
title_temp
...
You can also write this all together with a pipe:
...
4 step 
Using polite parkage using the scrape, bow 
1. Check if the scraping is allowed usng robotxt:: paths_allowed
2. Red HTMl, convert it into XML 
3. Identify nodes using html_nodes()
4. Covert to a tibble, using html_text(), html_table()
...
session <- bow("https://www.nih.gov/news-events/news-releases", force = TRUE)
  ...
  html_node(".thumbnail-teaser__description")|>
  html_text()
...
nih_top10 <- nih_top10 |>
  mutate(
    date = (myd(str_extract(description, ".+\\d{4}"))
  )
nih_top10
```
...
scrape_page <- function(url) {
  ...
  page <- scrape(session)
  nih_title <- get_text_from_page(page, ".thumbnail-teaser_heading")
  nih_description <- get_text_from_page(** FILL IN **)
...
```{r}
# check that scraping is allowed (Step 0)
robotstxt::paths_allowed("https://www.bestplaces.net/city/minnesota/minneapolis")

# Step 1: Download the HTML and turn it into an XML file with read_html()
best_places <- read_html("https://www.bestplaces.net/city/minnesota/minneapolis")

# Step 2: Extract specific nodes with html_nodes()
crime_temp <- html_nodes(best_places, ".col-4 > div:nth-child(1)")
crime_temp

# Step 3: Extract content from nodes with html_text()
#   Usually will still need to do some stringr adjustments
crime <- html_text(crime_temp) |>
  str_remove(" // 100")
parse_number(crime)


scrape_bestplaces <- function(state, city){
  url <- str_c("https://www.bestplaces.net/city/", state, "/", city, sep = "")
...
min_income_single <- scrape(session) |>
  html_nodes(".text-center+ .text-center div:nth-child(1)") |>
  html_text() |>
  parse_number()
min_income_single

rent_2br <- scrape(session) |>
...
3. Write a function called `scrape_bestplaces()` with arguments for `state` and `city`.  When you run, for example, `scrape_bestplaces("minnesota", "minneapolis")`, the output should be a 1 x 6 tibble with columns for `state`, `city`, `crime`, `min_income_single`, `rent_2br`, and `about`.


4. Create a 5 x 6 tibble by running `scrape_bestplaces()` 5 times with 5 cities you are interested in.  You might have to combine tibbles using `bind_rows()`.  Be sure you look at the URL at bestplaces.net for the various cities to make sure it works as you expect.  For bonus points, create the same 5 x 6 tibble for the same 5 cities using `purrr:map2`!
```{r}
city_data <- bind_rows(scrape_bestplaces("minnesota", "minneapolis"),
                       scrape_bestplaces("illinois", "chicago"),
                       scrape_bestplaces("washington", "seattle"),
                       scrape_bestplaces("texas", "austin"),
                       scrape_bestplaces("massachusetts", "boston"))
city_data

# Or use purrr::map2

# Create a character vector of states and cities
state_vec <- c("minnesota", 
               "illinois",
               "washington",
               "texas",
               "massachusetts")
city_vec <- c("minneapolis",
              "chicago",
              "seattle",
              "austin",
              "boston")

city_data2 <- purrr::map2(state_vec, city_vec, scrape_bestplaces)
bind_rows(city_data2)
```
