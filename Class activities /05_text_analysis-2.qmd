...
You can download this .qmd file from [here](https://github.com/joeroith/264_spring_2026/blob/main/05_text_analysis.qmd).  Just hit the Download Raw File button.
...
# write_csv(three_works, "~/264_spring_2026/Data/three_works.csv")
```

```{r}
# three_works <- read_csv("https://joeroith.github.io/264_spring_2026/Data/three_works.csv")
# three_works2 <- read_csv("Data/three_works.csv") 

library(RCurl)
three_works <- read_csv(
  file = getURL("https://raw.githubusercontent.com/joeroith/264_spring_2026/refs/heads/main/Data/three_works.csv", .encoding = "UTF-8"))
...
library(topicmodels)
three_books_lda <- LDA(three_books_dtm, k = 2, control = list(seed = 1234)) #k = 2 how many topic 
three_books_lda
```
...
beta_wide <- three_books_topics |>
  ...
  pivot_wider(names_from = topic, values_from = beta) |> #one term per line 
  filter(topic1 > .001 | topic2 > .001) |>
  mutate(log_ratio = log2(topic2 / topic1)) # log ratio
...
# potter_untidy <- read_csv("https://joeroith.github.io/264_spring_2026/Data/potter_untidy.csv") |>
# potter_untidy <- read_csv("Data/potter_untidy.csv")

potter_untidy <- read_csv(
  file = getURL("https://raw.githubusercontent.com/joeroith/264_spring_2026/refs/heads/main/Data/potter_untidy.csv", .encoding = "UTF-8")) |>
...
# potter_locations <- read_csv("https://joeroith.github.io/264_spring_2026/Data/potterlocations.csv") |>
# potter_locations <- read_csv("Data/potterlocations.csv")

potter_locations <- read_csv(
  file = getURL("https://raw.githubusercontent.com/joeroith/264_spring_2026/refs/heads/main/Data/potterlocations.csv", .encoding = "UTF-8")) |>
  mutate(value = str_to_lower(value))

potter_locations

# potter_names <- read_csv("https://joeroith.github.io/264_spring_2026/Data/potternames.csv") |>
# potter_names <- read_csv("Data/potternames.csv") |>
potter_names <- read_csv(
  file = getURL("https://raw.githubusercontent.com/joeroith/264_spring_2026/refs/heads/main/Data/potternames.csv", .encoding = "UTF-8")) |>
...
# potter_spells <- read_csv("https://joeroith.github.io/264_spring_2026/Data/potter_spells.csv") |>
# potter_spells <- read_csv("Data/potter_spells.csv") |>
potter_spells <- read_csv(
  file = getURL("https://raw.githubusercontent.com/joeroith/264_spring_2026/refs/heads/main/Data/potter_spells.csv", .encoding = "UTF-8")) |>
...
```{r}
words_smart <- potter_tidy |>
  anti_join(smart_stopwords, by = "word") |>
  filter(!is.na(word), word != "NA") |>
  count(word, sort = TRUE)

wordcloud(
  words = words_smart$word,
  freq  = words_smart$n,
  max.words = 150,
  random.order = FALSE
)

# optional: prettier HTML cloud
words_smart_df <- words_smart |>
  slice_max(n, n = 80) |>
  as.data.frame()

wordcloud2(words_smart_df, size = 0.5, shape = "circle")
```
...
# wordcloud2 can color by a column named "color"
sent_cloud <- top_sent_words |>
  mutate(color = if_else(sentiment == "positive", "green3", "red3")) |>
  transmute(word, freq = n, color) |>
  as.data.frame()

wordcloud2(sent_cloud, color = sent_cloud$color, size = 0.8)
```
...
potter_tidy |>
  ...
  ggplot(aes(x = chapter, y = net_sentiment)) +
  geom_col() +
  facet_wrap(~ title, scales = "free_x") +
  labs(
    title = "Net sentiment by chapter (bing): positive minus negative",
    x = "Chapter", y = "Net sentiment"
  )
```
...
potter_tidy |>
  ...
  ggplot(aes(x = chapter_factor, y = avg_sentiment)) +
  geom_point() +
  facet_wrap(~ title, scales = "free_x") 
...
```{r}
potter_word_counts <- potter_clean |>
  count(title, word, sort = TRUE)

potter_tfidf <- potter_word_counts |>
  bind_tf_idf(word, title, n)
...
potter_tfidf |>
  ...
  ungroup() |>
  ggplot(aes(x = fct_reorder(word, tf_idf), y = tf_idf)) +
  geom_col(show.legend = FALSE) +
  coord_flip() +
  facet_wrap(~ title, scales = "free") 
...
potter_bigrams_all <- potter_untidy |>
  unnest_tokens(bigram, text, token = "ngrams", n = 2) |>
  filter(!is.na(bigram), bigram != "NA") |>
  separate(bigram, c("word1", "word2"), sep = " ")

bigrams_separated <- potter_bigrams_all |>
  count(word1, word2, sort = TRUE) |>
  filter(!is.na(word1), !is.na(word2))
