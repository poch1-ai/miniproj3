...
z
z[1, ]
z[, 1] # I want to first columns and all the rows 
z[, -3] # give me all the collumn except the 3
```
...
class(mpg_model)
summary(mpg_model)# summary of each of the columns
```
...
```{r}
mean # good for working with date
```
...
data_sci$integers[c(3,1,4,1,5,9,3)]
data_sci[["intergers"]][c(3,1,4,1,5,9,3)]
```
...
typeof(data_sci$dont_forget(c[1,2,1,3]))
```
...
```{r}
data_sci[[4]][c(8,9)]
data_sci$our_first_tibble[c(8,9)]
data_sci[["our_first_tibble"]][c("hwy","cty")]
data_sci[[4]][c("hwy", "cty")]
data_sci$our_first_tibble[c("hwy", "cty")]
...
3. Predict what the following lines of R code will do, then check to see if you're right.  Do NOT run without making a prediction!!  If it's hard to see what's happening, create a small vector to test.

```{r}
#| eval: FALSE

# Group Challenge for class

1 + c(1, 2, 3)                                # 1
c(2, 2, 4, 4, 3) + c(1, 2, 3)                 # 2
tibble(x = c(2, 2, 4, 4, 3), y = c(1, 2, 3))  # 3
x <- c(2.3, 4.2, 7.1)
x[c(3, 2)]                     # 4
x[c(-1 * 2:3)]                 # 5
v <- 1:10
v[v %% 2 == 0]                 # 6
ifelse(c(TRUE, FALSE, FALSE, TRUE, FALSE), 1:5, 6:10)   # 7
g <- c("A", "A", "B", "B", "C")
ifelse(g == "A", 1, ifelse(g == "C", 2, 3))             # 8
z <- list(v1 = 1:5, v2 = c("a", "b", "c", "d", "e")) 
z[1] > 3                       # 9
z[[1]][2] * 3                  # 10
z$v2[z$v1 > 3]                 # 11
z[[c("v1", "v2")]]             # 12
