...
You can download this .qmd file from [here](https://github.com/joeroith/264_spring_2026/blob/main/02_strings_part1.qmd).
Just hit the Download Raw File button.

You can also see a video walkthrough of the first half of this worksheet [here](https://stolaf.hosted.panopto.com/Panopto/Pages/Viewer.aspx?id=dcdf1e3a-1f38-4686-87d6-b3ec00fca599)

This uses parts of R4DS Ch 14: Strings and Ch 15: Regular Expressions (both the first and second editions).

This dataset comes from the Tidy Tuesday community, [more details can be found here.](https://github.com/rfordatascience/tidytuesday/blob/main/data/2020/2020-01-21/readme.md)
...
## Functions that start str\_ do stuff to strings!

> Fun Fact: The `stringr` library in `tidyverse` holds all of the functions for working with strings.

### str_length()

```{r}
# when the input to str_length is a single string, the output is a single value:
str_length("hi/hehehe")
...
str_length takes a vector input and creates a vector output (or a single value input and returns a single value output)....
this makes it easy to use within a mutate!
...
How can we use str_sub to get just the year of the album_release_date?
Try it here!
Then scroll down for solution.

```{r}
spot_smaller |> 
  mutate(release_year = str_sub(album_release_date, 1, 4)) |> 
  select(title, album_release_date, release_year)
```
...
In addition to manipulating strings, we might want to search through them to find matches.
For example, can I find all the songs that start with M?
The songs from 2016?
The album titles that include a number?

### str_view()

This function is helpful for viewing.
It returns rows that contain the pattern you're searching for, highlighting the pattern between \<.\> symbols and in a different color.
...
str_subset() takes a vector input and returns a (usually shorter) vector output.
Compare the output from str_view() and str_subset() here.
Both of these functions can be hard to work with in a tibble.
...
str_detect takes a vector of strings (or single string) input and returns a vector of TRUE/FALSE (or single value).
This makes it easy to work with in tibbles, using mutate or filter.
...
```{r}
str_extract("find first vowel", "[aeiou]") # find the first vowel

str_extract("any numb3rs?", "\\d") #find the degit 
...
*Regular expressions* are a way to write general patterns...
for instance the string `\\d` will find any digit (number).
We can also specify whether we want the string to start or end with a certain letter.
...
But how do I look for a dollar sign in my string?
I use `\` to "escape" the special behavior of `$`.
But `\` itself has special behavior...
...
> See what I did with the dash ("-") here? Also notice the negating `^` applies to any of the following characters (including the space).
...
When multiple variables are crammed together into a single string, the separate\_ functions can be used to extract the pieces are produce additional rows (longer) or columns (wider).
We show one such example below, using the optional "too_few" setting to diagnose issues after getting a warning message the first time.
